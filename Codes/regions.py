...
class SpectraRegion():
    ...
    def loadLib(self):
        ...
        libfile = (glob.glob(os.path.join(baseFolder, "libPIMZ*.so")) + glob.glob(
            os.path.join(baseFolder, "../build/lib*/pIMZ/", "libPIMZ*.so")))[0]
        self.lib = ctypes.cdll.LoadLibrary(libfile)

        self.lib.StatisticalRegionMerging_New.argtypes = [ctypes.c_uint32, ctypes.POINTER(ctypes.c_float),
                                                          ctypes.c_uint8]
        self.lib.StatisticalRegionMerging_New.restype = ctypes.c_void_p

        self.lib.SRM_calc_similarity.argtypes = [ctypes.c_void_p, ctypes.c_uint32, ctypes.c_uint32,
    ...
    def to_aorta3d(self, folder, prefix, regionID, protWeights=None, nodf=False, pathPrefix=None, ctpred=None,
        ...
        if protWeights != None:
            ...
            if not nodf:
                markerGenes = self.find_all_markers(protWeights, use_methods=["ttest"], replaceExisting=False,
    ...
    def _fivenumber(self, valuelist, addfuncs=None):
        ...
        return tuple(
            [len(valuelist), len([x for x in valuelist if x > 0]), min_, quan25_, quan50_, quan75_, max_] + addRes)
    ...
    def plot_intensity_distribution(self, mass):
        ...
        print("Five Number stats + mean, var",
              self._fivenumber(allMassIntensities, addfuncs=[np.mean, lambda x: np.var(x) / np.mean(x)]))
    ...
    def calc_similarity(self, inputarray):
        ...
        outclust = np.ctypeslib.as_array(retValues, shape=(
        inputarray.shape[0] * inputarray.shape[1], inputarray.shape[0] * inputarray.shape[1]))
    ...
    def prepare_elem_matrix(self, dims=None):
        self.dimred_elem_matrix = np.zeros(
            (self.region_array.shape[0] * self.region_array.shape[1], self.region_array.shape[2]))
    ...
    def __segment__umap_hdbscan(self, number_of_regions, densmap=False, dims=None, n_neighbors=10, min_cluster_size=20,
                                num_samples=10000):
    ...
    def _calculate_sa_vector(self, array, current_spectrum, currentPos, radius, sigma):
        ...
        for i in range(-radius, radius + 1):
            for j in range(-radius, radius + 1):
                ...
                if neighbor[0] >= 0 and neighbor[1] >= 0 and neighbor[0] < self.region_array.shape[0] and neighbor[1] < \
        ...
        for x in range(self.region_array.shape[0]):
            for y in range(self.region_array.shape[1]):
                ...
                for i in range(-radius, radius + 1):
                    for j in range(-radius, radius + 1):
                        neighbor = (x + i, y + j)
                        if neighbor[0] >= 0 and neighbor[1] >= 0 and neighbor[0] < self.region_array.shape[0] and \
                                neighbor[1] < self.region_array.shape[1]:
                            weight = math.exp(-i ** 2 - j ** 2) / (2 * sigma ** 2)
                            f.append(
                                np.dot(curr_spectrum, self.region_array[neighbor[0]][neighbor[1]]) * math.sqrt(weight))
                        else:
                            f.append(0.0)
                output_matrix[x][y] = f
    ...
    def get_beta(x, y, i, j, matrix, radius):
        if x + i >= 0 and y + j >= 0 and x + i < matrix.shape[0] and y + j < matrix.shape[1]:
            ...
            delta = 0
        deltas = [np.linalg.norm(matrix[x + r1][y + r2] - matrix[x][y]) for r1 in range(-radius, radius) for r2 in
                  range(-radius, radius) if
                  x + r1 >= 0 and y + r2 >= 0 and x + r1 < matrix.shape[0] and y + r2 < matrix.shape[1]]
                ...
                for i in range(-radius, radius + 1):
                    for j in range(-radius, radius + 1):
                        ...
                        if neighbor[0] >= 0 and neighbor[1] >= 0 and neighbor[0] < self.region_array.shape[0] and \
                            ...
                            new_weight = old_weight * math.sqrt(
                                self.get_beta(x, y, i, j, self.region_array, radius) * self.get_beta(neighbor[0],
                                                                                                     neighbor[1], i, j,
                                                                                                     self.region_array,
                                                                                                     radius))
                            f.append(np.dot(curr_spectrum, self.region_array[neighbor[0]][neighbor[1]]) * math.sqrt(
                                new_weight))
                        else:
                            f.append(0.0)
                output_matrix[x][y] = f
    ...
    def plot_volcano(self, method, comparison, title, outfile=None, topn=30, masses=None, gene_names=None,
        ...
        """
        dataframe = pd.merge(self.df_results_all[method][comparison], self.de_results_all[method][comparison],
                             left_on=['gene_ident'], right_on=['gene'])
    ...
    def _plot_volcano(FcPvalGene, title, outfile=None, showGeneCount=30, showGene=None):
        ...
        with plt.style.context("default"):
            ...
            plt.vlines(x=-0.5, ymin=pvalThresh, ymax=yMaxLim, linestyle="dotted")
            adjust_text(texts, force_points=0.2, force_text=0.2, expand_points=(2, 2), expand_text=(1, 1),
    ...
    def segment(self, method="UPGMA", dims=None, number_of_regions=10, n_neighbors=10, min_cluster_size=20,
                num_samples=1000):
        ...
        if method == "UPGMA":
            ...
            c = self.__segment__umap_hdbscan(number_of_regions, dims=dims, n_neighbors=n_neighbors,
                                             min_cluster_size=min_cluster_size, num_samples=num_samples)
    ...
    def mass_dabest(self, masses, background=0):
        ...
        for mass in masses:
            ...
            dfObj = pd.DataFrame(
                {"mass": massVec, "specidx": specIdxVec, "cluster": clusterVec, "intensity": intensityVec})
        ...
        for i in range(img.shape[0]):
            for j in range(img.shape[1]):
                if img[i, j] in background:
                    ...
                    if not self.__check_neighbour(img, i, j, background) and self.__check_neighbour(cartoon_img, i, j,
                                                                                                    aorta) or img[i][
                        j] in plaque:
                        cartoon_img[i, j] = 2
                    else:
...
document.addEventListener('readystatechange', event => {
    ...
    def export_deres(self, method, resKey, outpath, title="DE Result"):
        ...
        self.logger.info(
            "Adding Mass Heatmap at pos {} of {} with {} entries".format(pos, len(expDF.columns.values.tolist()),
                                                                         len(massImgValues)))
    ...
    def deres_to_df(self, method, resKey, protWeights, mz_dist=3, mz_best=False, keepOnlyProteins=True,
                    inverse_fc=False, max_adj_pval=0.05, min_log2fc=0.5):
        ...
        self.logger.info("Created matrices with shape {} and {} (target, bg)".format(targetSpectraMatrix.shape,
                                                                                     bgSpectraMatrix.shape))
        ...
        for row in fttr.iterrows():
            ...
            expT, totalSpectra, measuredSpecta = self.get_expression_from_matrix(targetSpectraMatrix, massValue,
                                                                                 resKey[0], ["avg", "median"])
            exprBG, totalSpectraBG, measuredSpectaBG = self.get_expression_from_matrix(bgSpectraMatrix, massValue,
                                                                                       resKey[0], ["avg", "median"])
    ...
    def find_all_markers(self, protWeights, keepOnlyProteins=True, replaceExisting=False, includeBackground=True,
                         mz_dist=3, mz_best=False, backgroundCluster=[0], out_prefix="nldiffreg", outdirectory=None,
                         use_methods=["empire", "ttest", "rank"], count_scale={"ttest": 1, "rank": 1, "empire": 10000}):
        ...
        for segment in cluster2coords:
            ...
            self.find_markers(clusters0=clusters0, clusters1=clusters1, replaceExisting=replaceExisting,
                              outdirectory=outdirectory, out_prefix=out_prefix, use_methods=use_methods,
            ...
            for method in keyResults:
                ...
                resDF = self.deres_to_df(method, resKey, protWeights, keepOnlyProteins=keepOnlyProteins,
                                         inverse_fc=inverseFC, mz_dist=mz_dist, mz_best=mz_best)
    ...
    def run_nlempire(self, nlDir, pdata, pdataPath, diffOutput):
        ...
        if mc[1] > 100:
            code, out, err = run(["/usr/bin/java", "-Xmx24G", "-cp", pysrmPath + "/../../../tools/nlEmpiRe.jar",
                                  "nlEmpiRe.input.ExpressionSet", "-getbestsubN", "100", "-inputdir", nlDir, "-cond1",
                                  "0", "-cond2", "1", "-o", nlDir + "/exprs_ds.txt"])
        ...
        runI = 0
        while runI < 10:
            code, out, err = run(["/usr/bin/java", "-Xmx16G", "-cp", pysrmPath + "/../../../tools/nlEmpiRe.jar",
                                  "nlEmpiRe.input.ExpressionSet", "-inputdir", nlDir, "-cond1", "0", "-cond2", "1",
                                  "-o", diffOutput])
    ...
    def find_markers(self, clusters0, clusters1=None, out_prefix="nldiffreg", outdirectory=None, replaceExisting=False,
                     use_methods=["empire", "ttest", "rank"], count_scale={"ttest": 1, "rank": 1, "empire": 10000},
                     sample_max=-1):
        ...
        if outdirectory != None and "empire" in use_methods:
            ...
            if replaceExisting or fillCondition:
                ...
                nlOutput = outdirectory + "/" + out_prefix + "." + "_".join(
                    [str(z) for z in resKey[0]]) + "." + "_".join([str(z) for z in resKey[1]]) + ".tsv"
        ...
        if len(diffxPyTests.intersection(use_methods)) > 0:
            ...
            for testMethod in use_methods:
                ...
                if replaceExisting or fillCondition:
                    ...
                    self.logger.info(
                        "Skipping {} for: {}, {}, {}".format(testMethod, resKey, replaceExisting, fillCondition))
...
class ProteinWeights():
    ...
    def __init__(self, filename, min_mass=-1, max_mass=-1):
        ...
        if filename != None:
            ...
            self.logger.info(
                "Loaded a total of {} proteins with {} masses".format(len(self.protein2mass), len(allMasses)))
    ...
    def print_collisions(self, maxdist=2.0, print_proteins=False):
        ...
        self.logger.info("        Mean Number of collisions: {}".format(
            np.mean([protsWithCollision[x] for x in protsWithCollision])))
        self.logger.info("      Median Number of collisions: {}".format(
            np.median([protsWithCollision[x] for x in protsWithCollision])))
